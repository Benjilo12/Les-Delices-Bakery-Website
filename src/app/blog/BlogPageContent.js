        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-24">
            {blogs.map((blog) => (
              <Link
                key={blog._id}
                href={`/blog/${blog.slug}`} // Fixed: Changed from `/blog/${blog.slug}` to `/blogs/${blog.slug}`
        ...
        {pagination.totalPages > 1 && (
          <div className="mt-32 flex justify-center items-center gap-12 border-t border-gray-100 pt-16">
            <button
              ...
              className="group p-2 disabled:opacity-10 hover:text-amber-700 transition-all disabled:cursor-not-allowed"
            >
